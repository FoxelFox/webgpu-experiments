...
fn main(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
	...
	for (var i = 0i; i < i32(myUniform.edgesTextureSize); i++) {
        ...
        if (edge.z > 0.1) {
            ...
            var dis = distance(ePos, vPos) + 0.000000001;
            vVel += (ePos - vPos) * dis * 0.001;
        }
	}
	...
	var uv = vec2i(
		i32((( vPos.x + 1.0) * 0.5) * 128.0),
		i32(((-vPos.y + 1.0) * 0.5) * 128.0)
	);
	...
	if (distanceToEnd > 0.99) {
		offset += (vec2(0) - vPos) * pow(distanceToEnd + 0.001, 2.0) * 0.0001;
	}
